...
| Review comments had nowhere to go | Questions capture against a file and line, and reach agents over MCP |
| Everything too small to read comfortably | Zoom on the usual shortcuts, persisted across restarts |
| An agent could not tell which pull request its questions belonged to | The payload names the branch, title, head commit, and stack position |
| An agent had to guess pull request numbers to find questions on a sibling branch | Opening a pull request records every member of its stack |
| Three fixed columns squeezed the diff | Panels resize by dragging, and questions dock beside the diff or beneath it |
