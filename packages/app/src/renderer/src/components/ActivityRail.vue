...
defineProps<{ active: "explorer" | "changes" | "pulls" | "settings"; localOpen: boolean }>();
...
<template>
  <nav class="rail" aria-label="Workspace views">
    <button v-for="action in actions" :key="action.id" :class="{ active: active === action.id }" :disabled="action.id !== 'pulls' && !localOpen" :aria-label="action.label" :title="action.label" @click="emit('select', action.id)">
